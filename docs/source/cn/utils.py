...
reset_docstr(
    ...
    参数：
        ...
        - **prefetch_factor** (int, 可选, 键word-only arg): 在每个worker之前就被加载的样本数量 ``2``意味着将会有 2 * num_workers 个样本在所有worker之前被预提取。(default: ``2``)
...
reset_docstr(
    ...
    所有包含可迭代的数据样本的数据集都应作为此类的子类。此类数据集在数据来源为数据流时尤其有用。
