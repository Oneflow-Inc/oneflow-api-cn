...
reset_docstr(
    ...
    参数：
        - **name** (string) - 子模块的名称，可以使用给定的名称作为属性来访问模块。
...
reset_docstr(
    ...
    这也会使关联的参数和缓存成为不同的对象。因此，如果模块运行在 GPU 上且需要被优化，该函数应在构建优化器之前被调用。

    .. note::
        此方法会以 in-place 方式修改模块。

    参数：
        - **device** (int, optional) - 如果指定了 device，所有参数将被复制到此
...
reset_docstr(
    ...
    """设置此模块的其他表现方式。若要打印自定义的额外信息，应在此模块中重新实现此方法。接受单行和多行字符串。
    """
)
