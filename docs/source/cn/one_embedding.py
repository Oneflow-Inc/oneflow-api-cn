...
reset_docstr(
    ...
    r"""MultiTableEmbedding 代表具有相同 embedding_dim、dtype 和 key_type 的多个 embedding 表。

    参数：
        - **name** (str) - Embedding 的名字。
        ...
        - **default_initializer** (dict, optional) - 如果表参数为 None，使用 default_initializer 来初始化表，默认为 None。
...
reset_docstr(
    ...
    参数：       
        - **snapshot_name** (str) - snapshot_name，快照将被保存在 your_configed_persistent_path 目录下。
...
reset_docstr(
    ...
    """加载快照。
        ...
        参数：
            - **snapshot_name** (str) - snapshot_name，快照将从 your_configed_persistent_path 下加载。
...
reset_docstr(
    ...
    """MultiTableEmbedding 的前向计算。
        ...
        参数：
            - **ids** (flow.tensor) - 特征的id。
            - **table_ids** (flow.tensor, optional) - 每个 table_id 的 id 必须与 ids 的形状相同。如果只配置了一个表或者 id 的形状为（batch_size, num_tables），并且每个列的 id 都属于表的 column_id，就没有必要传递 table_ids，否则，应该传递 tensor_ids。
