...
reset_docstr(
    ...
    参数：
        - **input** - 形状为 :math:`(\text{minibatch} , \text{in_channels} , iD , iH , iW)` 的量化输入张量
        - **weight** - 形状为 :math:`(\text{out_channels} , \frac{\text{in_channels}}{\text{groups}} , kD , kH , kW)` 的量化滤波器
        - **bias** - 形状为 :math:`(\text{out_channels})` 的化偏置张量，其类型必须为 `flow.float`。
