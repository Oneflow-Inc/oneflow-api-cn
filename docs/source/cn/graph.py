...
reset_docstr(
    ...
    r"""在 Graph 中打开或关闭 debug 模式。
        ...
        参数：
            ...
            - **ranks** (int or list(int))- 选择排名以打印调试信息， 默认等级为 ``0`` 。
                你可以选择任何有效的等级。Ranks 等于 ``-1`` 则表示在所有等级上调试。
            - **mode** (bool)-m 是否设置调试模式 (``True``) 或者 (``False``)。 默认值： ``True``。
        """
)
